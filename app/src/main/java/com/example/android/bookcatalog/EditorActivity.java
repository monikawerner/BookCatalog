...
import com.example.android.bookcatalog.data.BookContract;
import com.example.android.bookcatalog.data.BookDbHelper;
...
public class EditorActivity extends AppCompatActivity {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    if (selection.equals(getString(R.string.genre_thriller))) {
                        mGenre = BookContract.BookEntry.GENRE_THRILLER; // Thriller
                    } else if (selection.equals(getString(R.string.genre_guide))) {
                        mGenre = BookContract.BookEntry.GENRE_GUIDE; // Guide
                    } else if (selection.equals(getString(R.string.genre_novel))) {
                        mGenre = BookContract.BookEntry.GENRE_NOVEL; // Novel
                    } else if (selection.equals(getString(R.string.genre_biography))) {
                        mGenre = BookContract.BookEntry.GENRE_BIOGRAPHY; // Biography
                    } else if (selection.equals(getString(R.string.genre_for_kids))) {
                        mGenre = BookContract.BookEntry.GENRE_FOR_KIDS; // For kids
                    } else if (selection.equals(getString(R.string.genre_fantasy))) {
                        mGenre = BookContract.BookEntry.GENRE_FANTASY; // Fantasy
                    } else if (selection.equals(getString(R.string.genre_romance))) {
                        mGenre = BookContract.BookEntry.GENRE_ROMANCE; // Romance
                    } else if (selection.equals(getString(R.string.genre_scientific))) {
                        mGenre = BookContract.BookEntry.GENRE_SCIENTIFIC; // Scientific
                    } else {
    ...
    private void insertBook() {
        ...
        values.put(BookContract.BookEntry.COLUMN_GENRE, mGenre);
        values.put(BookContract.BookEntry.COLUMN_PUBLISHING_HOUSE, publishingHouseString);
        values.put(BookContract.BookEntry.COLUMN_PUBLICATION_DATE, publicationDate);
        values.put(BookContract.BookEntry.COLUMN_LOCALIZATION, locationString);
